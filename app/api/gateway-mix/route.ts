...
import { gatewayMixQuery, type GatewayMixRow } from '@/lib/bq/templates/gateway-mix';
...
export async function POST(req: Request) {
  ...
  try {
    const bq = getBQ();
    const { query, params, types } = gatewayMixQuery(filters);
    ...
    return NextResponse.json({
      data: rows as GatewayMixRow[],
      generatedAt: new Date().toISOString(),
    });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(JSON.stringify({ handler: '/api/gateway-mix', error: message }));
