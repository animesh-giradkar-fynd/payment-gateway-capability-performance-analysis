...
export async function POST(req: Request) {
  ...
  try {
    const bq = getBQ();
    const { query, params, types } = failuresQuery(filters);
    ...
    return NextResponse.json({
      data: rows as FailureReasonRow[],
      generatedAt: new Date().toISOString(),
    });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(JSON.stringify({ handler: '/api/failures', error: message }));
